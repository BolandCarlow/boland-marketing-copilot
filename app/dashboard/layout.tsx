...
export default async function DashboardLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <div className="min-h-screen bg-slate-950 md:flex">
    <aside className="border-b border-slate-800 bg-slate-900 p-6 md:min-h-screen md:w-64 md:border-b-0 md:border-r">
      ...
      <nav className="mt-10 space-y-2 text-sm"><Link className="block rounded-lg bg-slate-800 px-3 py-2" href="/dashboard">Overview</Link><a className="block rounded-lg px-3 py-2 text-slate-400">Campaigns <span className="float-right text-xs">Soon</span></a><a className="block rounded-lg px-3 py-2 text-slate-400">Reports <span className="float-right text-xs">Soon</span></a></nav>
