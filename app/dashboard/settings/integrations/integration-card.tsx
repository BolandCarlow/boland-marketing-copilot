...
export function IntegrationCard({ provider, current }: { provider: ProviderDefinition; current: CurrentIntegration }) {
  ...
  return <article className="card integration-card">
    ...
    <p>{provider.description}</p>{provider.id === "meta-ads" ? <Link className="button primary" href="/dashboard/settings/integrations/meta-ads">Manage Meta Ads</Link> : null}<div className="flex flex-wrap gap-2">{provider.capabilities.map((capability) => <span className="pill" key={capability}>{capability}</span>)}</div>
