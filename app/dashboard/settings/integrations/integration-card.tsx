...
export function IntegrationCard({ provider, current }: { provider: ProviderDefinition; current: CurrentIntegration }) {
  ...
  const isMetaAds = provider.id === "meta-ads"; const connected = current.connectionStatus === "connected";
  return <article className="card integration-card">
    ...
    <p>{provider.description}</p>{isMetaAds ? <Link className="button primary" href="/dashboard/settings/integrations/meta-ads">{connected ? "Manage Meta Ads" : "Connect Meta"}</Link> : null}<div className="flex flex-wrap gap-2">{provider.capabilities.map((capability) => <span className="pill" key={capability}>{capability}</span>)}</div>
    ...
    {state.message && !isMetaAds ? <p className="notice" role="status">{state.message}</p> : null}
  </article>;
}
