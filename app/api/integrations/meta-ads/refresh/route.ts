...
export async function POST() {
  try {
    ...
    return NextResponse.json(await refreshMetaAccessToken(connection.id, connection.encrypted_access_token));
  } catch (exception) {
    return NextResponse.json({ error: exception instanceof Error ? exception.message : "Meta token refresh failed." }, { status: 500 });
  }
}
