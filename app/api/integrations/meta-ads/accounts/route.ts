...
export async function GET() {
  try {
    ...
    return NextResponse.json({ businesses: await discoverMetaBusinesses(decryptMetaCredential(connection.encrypted_access_token)) });
  } catch (exception) {
    return NextResponse.json({ error: exception instanceof Error ? exception.message : "Meta Business Managers could not be loaded." }, { status: 500 });
  }
}
