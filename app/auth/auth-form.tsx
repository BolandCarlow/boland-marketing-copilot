...
import { AUTH_CALLBACK_URL } from "@/lib/supabase/config";
...
export function AuthForm({ initialMessage }: { initialMessage?: string }) {
  ...
  async function signIn(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy("email"); setMessage(""); const { error } = await createClient().auth.signInWithOtp({ email, options: { emailRedirectTo: AUTH_CALLBACK_URL } }); setMessage(error ? error.message : "Check your inbox for a secure sign-in link."); setBusy(null); }
  async function signInWithGoogle() { setBusy("google"); setMessage(""); const { error } = await createClient().auth.signInWithOAuth({ provider: "google", options: { redirectTo: AUTH_CALLBACK_URL } }); if (error) { setMessage(error.message); setBusy(null); } }
